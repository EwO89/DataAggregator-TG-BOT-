...
class BotHandlers:
    def __init__(self, bot: Bot, users: AsyncIOMotorCollection, data_collection: AsyncIOMotorCollection) -> None:
        self.bot = bot
        self.users = users
    ...
    async def get_json(self, message: Message, state: FSMContext) -> None:
...
def setup_handlers(dp: Dispatcher, bot: Bot, users: AsyncIOMotorCollection,
                   data_collection: AsyncIOMotorCollection) -> None:
    handlers = BotHandlers(bot, users, data_collection)
    dp.message.register(handlers.start, Command(commands=["start"]))
    dp.message.register(handlers.get_json, MyStates.waiting_for_json)
